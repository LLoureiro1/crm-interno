-- Ajuste de pesos (teto 100), desistente=0, recálculo em lote
...
BEGIN
  ...
  -- Recência: último engajamento real (contato bem-sucedido, e-mail aberto, atendimento/prova)
  SELECT GREATEST(
    COALESCE((
    ...
    COALESCE((
      ...
      FROM public.email_queue eq
      WHERE eq.student_id = p_student_id AND eq.opened_at IS NOT NULL
    ), '-infinity'::timestamptz),
    COALESCE((
    ...
    COALESCE((
      ...
      FROM public.student_interactions si
      WHERE si.student_id = p_student_id
        AND si.interaction_type IN ('atendimento', 'agendamento_entrevista', 'reagendamento', 'email_opened')
    ), '-infinity'::timestamptz)
  ) INTO v_last_touch;

  IF v_last_touch = '-infinity'::timestamptz THEN
    v_days_since_touch := GREATEST(
      0,
  ...
  ELSE
    v_days_since_touch := GREATEST(
      0,
      FLOOR(EXTRACT(EPOCH FROM (now() - v_last_touch)) / 86400)::int
    );
  END IF;
  ...
  ELSIF v_days_since_touch <= 14 THEN
    v_recencia := 1;
  ELSIF v_days_since_touch <= 30 THEN
    v_recencia := -2;
  ELSIF v_days_since_touch <= 60 THEN
    v_recencia := -4;
  ELSIF v_days_since_touch <= 90 THEN
    v_recencia := -6;
  ELSE
    v_recencia := -8;
  END IF;
...
-- Recalcular scores existentes com a nova lógica
