-- Próximo exame válido (fuso America/Sao_Paulo): ignora provas do dia já encerradas
...
AS $$
  ...
  WHERE ed.unit_id = p_unit_id
    AND (
      ed.exam_date > timezone('America/Sao_Paulo', now())::date
      OR (
        ed.exam_date = timezone('America/Sao_Paulo', now())::date
        AND ed.exam_time > timezone('America/Sao_Paulo', now())::time
      )
    )
...
BEGIN
  ...
  IF EXISTS (
    ...
    SELECT n.id INTO NEW.exam_date_id
