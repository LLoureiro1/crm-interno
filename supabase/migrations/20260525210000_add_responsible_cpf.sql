-- CPF do responsável na inscrição
...
GRANT EXECUTE ON FUNCTION public.validate_cpf(text) TO anon, authenticated;
...
BEGIN
  ...
  IF v_responsible_cpf = '' THEN
    RETURN jsonb_build_object('success', false, 'error', 'CPF do responsável é obrigatório');
  END IF;

  IF NOT public.validate_cpf(v_responsible_cpf) THEN
