-- Listagem rápida de escolas: evita RLS por linha (user_has_student_access)
-- que causava statement timeout com dezenas de milhares de registros.
...
-- Policy otimizada: (SELECT ...) força initPlan (avalia 1x por query, não por linha)
...
TO authenticated
USING (
  (SELECT public.auth_user_is_admin())
  OR EXISTS (
    SELECT 1
...
GRANT EXECUTE ON FUNCTION public.list_schools_page(integer, integer, integer[], text[], text, uuid[], text, boolean, text) TO authenticated, service_role;

COMMENT ON FUNCTION public.list_schools_page IS
  'Lista escolas paginadas sem avaliar RLS por linha; checa acesso do usuário uma vez.';
