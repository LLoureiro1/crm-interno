...
DECLARE
  ...
  v_series_id uuid;
  v_class_id uuid;
  v_token_student_id uuid;
  v_token uuid := gen_random_uuid();
  v_ineligible_id uuid;
  v_ineligible_token uuid := gen_random_uuid();
  v_success_id uuid;
...
BEGIN
  ...
  INSERT INTO public.students (
    ...
    'Aluno Token', 'Resp', '11999999993', 'token@smoke.test',
    'São Paulo', 'Centro', 'Escola', '2010-01-01', v_class_id, v_unit_id,
    'nenhum_agendamento', v_token, now()
  ) RETURNING id INTO v_token_student_id;

  v_result := public.public_schedule_interview(
    v_token_student_id, v_interviewer_id, v_future_date, '10:00'::time,
  ...
  -- Cenário 2: status inelegível (matriculado) — aluno separado
  INSERT INTO public.students (
    student_name, responsible_name, phone, email, city, neighborhood,
    origin_school, birth_date, class_id, unit_id, status, registration_token, created_at
  ) VALUES (
    'Aluno Matriculado', 'Resp', '11999999994', 'matriculado@smoke.test',
    'São Paulo', 'Centro', 'Escola', '2010-01-01', v_class_id, v_unit_id,
    'matriculado', v_ineligible_token, now()
  ) RETURNING id INTO v_ineligible_id;

  v_result := public.public_schedule_interview(
    v_ineligible_id, v_interviewer_id, v_future_date, '10:00'::time, v_ineligible_token, NULL
  );
  ...
  -- Cenário 3: agendamento válido — aluno separado, nunca matriculado
  INSERT INTO public.students (
    student_name, responsible_name, phone, email, city, neighborhood,
    origin_school, birth_date, class_id, unit_id, status, registration_token, created_at
  ) VALUES (
    'Aluno Agendamento', 'Resp', '11999999995', 'agenda@smoke.test',
    'São Paulo', 'Centro', 'Escola', '2010-01-01', v_class_id, v_unit_id,
    'nenhum_agendamento', v_success_token, now()
  ) RETURNING id INTO v_success_id;

  v_result := public.public_schedule_interview(
    v_success_id, v_interviewer_id, v_future_date, '10:00'::time, v_success_token, NULL
  );
  PERFORM smoke_test.assert_true((v_result->>'success')::boolean, 'agendamento válido deve passar');
  PERFORM smoke_test.assert_eq(
    (SELECT status::text FROM public.students WHERE id = v_success_id),
  ...
  PERFORM smoke_test.assert_true(
    EXISTS (
      ...
      WHERE student_id = v_success_id AND status = 'scheduled'
    ),
  ...
  PERFORM smoke_test.assert_true(
    EXISTS (
      ...
      WHERE student_id = v_success_id AND interaction_type = 'agendamento_entrevista'
    ),
